...
parser.add_argument(
    ...
    help="GCC/Clang stderr log (generated with " "-fmessage-length=0)",
)
parser.add_argument(
    "-e",
    "--exclude",
...
with args.input as f:
    for line in f:
        ...
        if ": warning:" in line and re.match(
            "(\w*\/)*\S*\.(h|hh|H|hxx|hpp|h\+\+|c|cc|C|cxx|c\+\+|cpp):", line
        ):
...
if args.exclude:
    ...
    for key, value in d_warn.items():
        d_out[key] = [
            s for s in value if not any(e in s.split(":", 1)[0] for e in args.exclude)
        ]
...
for key, value in sorted(d_out.items(), key=lambda v: len(v[1]), reverse=True):
    s = "{warning:<{width}} {n}\n".format(width=max_len_w, warning=key, n=len(value))
    f.write(s)
